...
public sealed class McpToolSchemaFixTests : IDisposable
{
    ...
    public McpToolSchemaFixTests()
    {
        ...
        _registry = new McpToolRegistry(
            ...
            new StubIndexingEngine(),
            new StubIndexQueryService());
    }
    ...
    {
        public Task<SnapshotMetadata?> GetSnapshotInfoAsync(string? snapshotId, string? rootPath,
            CancellationToken ct = default)
        {
            return Task.FromResult<SnapshotMetadata?>(null);
        }

        public Task<IndexSnapshot?> GetSnapshotAsync(string? snapshotId, string? rootPath,
            CancellationToken ct = default)
        {
            return Task.FromResult<IndexSnapshot?>(null);
        }

        public Task<ListFilesResponse> ListFilesAsync(ListFilesRequest request, CancellationToken ct = default)
        {
            return Task.FromResult(new ListFilesResponse { SnapshotId = "stub" });
        }

        public Task<FileOutlineResponse?> GetFileOutlineAsync(GetFileOutlineRequest request,
            CancellationToken ct = default)
        {
            return Task.FromResult<FileOutlineResponse?>(null);
        }
        ...
        {
            return Task.FromResult(new FindSymbolsResponse { SnapshotId = "stub" });
        }

        public Task<GetSymbolResponse> GetSymbolAsync(GetSymbolRequest request, CancellationToken ct = default)
        {
            return Task.FromResult(new GetSymbolResponse { SnapshotId = "stub" });
        }

        public Task<GetSymbolOccurrencesResponse> GetSymbolOccurrencesAsync(GetSymbolOccurrencesRequest request,
            int page = 1, int pageSize = 50, CancellationToken ct = default)
        {
            return Task.FromResult(
                new GetSymbolOccurrencesResponse { SnapshotId = "stub", SymbolId = request.SymbolId });
        }

        public Task<GetSymbolChildrenResponse> GetSymbolChildrenAsync(GetSymbolChildrenRequest request, int page = 1,
            int pageSize = 50, CancellationToken ct = default)
        {
            return Task.FromResult(new GetSymbolChildrenResponse { SnapshotId = "stub", SymbolId = request.SymbolId });
        }

        public Task<GetTypeMembersResponse> GetTypeMembersAsync(GetTypeMembersRequest request,
            CancellationToken ct = default)
        {
            return Task.FromResult(new GetTypeMembersResponse
                { SnapshotId = "stub", TypeSymbolId = request.TypeSymbolId });
        }

        public Task<ResolveLocationResponse> ResolveLocationAsync(ResolveLocationRequest request,
            CancellationToken ct = default)
        {
            return Task.FromResult(new ResolveLocationResponse { SnapshotId = "stub" });
        }

        public Task<GetNamespaceTreeResponse> GetNamespaceTreeAsync(GetNamespaceTreeRequest request,
            CancellationToken ct = default)
        {
            return Task.FromResult(new GetNamespaceTreeResponse { SnapshotId = "stub" });
        }

        public Task<GetImportsResponse> GetImportsAsync(GetImportsRequest request, int page = 1, int pageSize = 50,
            CancellationToken ct = default)
        {
            return Task.FromResult(new GetImportsResponse
                { SnapshotId = "stub", FilePath = request.FilePath, Error = "not implemented" });
        }

        public Task<GetReferencesResponse> GetReferencesAsync(GetReferencesRequest request, int page = 1,
            int pageSize = 50, CancellationToken ct = default)
        {
            return Task.FromResult(new GetReferencesResponse { SnapshotId = "stub", SymbolId = request.SymbolId });
        }

        public Task<GetFileReferencesResponse> GetFileReferencesAsync(GetFileReferencesRequest request, int page = 1,
            int pageSize = 50, CancellationToken ct = default)
        {
            return Task.FromResult(new GetFileReferencesResponse
                { SnapshotId = "stub", FilePath = request.FilePath, Error = "not implemented" });
        }

        public Task<GetDependenciesResponse> GetDependenciesAsync(GetDependenciesRequest request, int page = 1,
            int pageSize = 50, CancellationToken ct = default)
        {
            return Task.FromResult(new GetDependenciesResponse { SnapshotId = "stub", SymbolId = request.SymbolId });
        }

        public Task<GetDependentsResponse> GetDependentsAsync(GetDependentsRequest request, int page = 1,
            int pageSize = 50, CancellationToken ct = default)
        {
            return Task.FromResult(new GetDependentsResponse { SnapshotId = "stub", SymbolId = request.SymbolId });
        }

        public Task<GetSymbolRelationshipsResponse> GetSymbolRelationshipsAsync(GetSymbolRelationshipsRequest request,
            CancellationToken ct = default)
        {
            return Task.FromResult(new GetSymbolRelationshipsResponse
                { SnapshotId = "stub", SymbolId = request.SymbolId });
        }

        public Task<GetFileDependenciesResponse> GetFileDependenciesAsync(GetFileDependenciesRequest request,
            CancellationToken ct = default)
        {
            return Task.FromResult(new GetFileDependenciesResponse
                { SnapshotId = "stub", FilePath = request.FilePath, Error = "not implemented" });
        }
    }
}