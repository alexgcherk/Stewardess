...
public sealed class RepoBrowserToolTests : IDisposable
{
    ...
    public void PrintTree_IncludeFilesAndDirs_DefaultToTrue()
    {
        ...
        Assert.Equal(true.ToString(), files.Default?.ToString(), true);
        Assert.Equal(true.ToString(), dirs.Default?.ToString(), true);
    }
    ...
    public void ReadFile_IncludeLineNumbers_DefaultsToTrue()
    {
        ...
        Assert.Equal(true.ToString(), prop.Default?.ToString(), true);
    }
    ...
    {
        var result = await InvokeAsync(ReadFile,
            new { file_path = "src/MyLib/Class1.cs", include_line_numbers = true });
        var content = result["content"]?.Value<string>() ?? "";

        Assert.True(content.StartsWith("1: "),
            $"Content should start with '1: ' but was: {content.Substring(0, Math.Min(30, content.Length))}");
    }
    ...
    {
        var result = await InvokeAsync(ReadFile,
            new { file_path = "src/MyLib/Class1.cs", include_line_numbers = false });
        var content = result["content"]?.Value<string>() ?? "";
