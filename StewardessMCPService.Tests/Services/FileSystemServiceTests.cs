...
namespace StewardessMCPService.Tests.Services
{
    ...
    public sealed class FileSystemServiceTests : IDisposable
    {
        ...
        public async Task ListTree_MaxDepthZero_ReturnsOnlyRootWithNoChildren()
        {
            ...
            Assert.NotNull(result.Root);
            Assert.Empty(result.Root.Children ?? new System.Collections.Generic.List<StewardessMCPService.Models.TreeNode>());
        }
        ...
        private static void AssertNoFileNodes(StewardessMCPService.Models.TreeNode node)
        {
            if (node == null) return;
            foreach (var child in node.Children ?? new System.Collections.Generic.List<StewardessMCPService.Models.TreeNode>())
            {
