...
public sealed class CodeIndexContractTests : IClassFixture<ReferenceIndexedFixture>
{
    ...
    {
        return (token as JObject)?.GetValue(key, StringComparison.OrdinalIgnoreCase)?.Value<string>();
    }
    ...
    public async Task Confidence_Dependencies_HaveResolutionMethodAndConfidence()
    {
        ...
        foreach (var dep in deps)
        {
            ...
            var conf = (dep as JObject)?.GetValue("Confidence", StringComparison.OrdinalIgnoreCase)?.Value<double>() ??
                       -1.0;
        ...
        foreach (var section in new[] { "Children", "References", "Dependencies", "Dependents" })
        {
            ...
            if (items != null)
                Assert.True(items.Count <= 5,
                    $"Section '{section}' must have at most 5 items in summary mode, got {items.Count}");
        }
    }
}