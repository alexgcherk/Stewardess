...
public sealed class CodeIndexIncrementalTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        WriteFile("Greeting.cs",
            "namespace Demo { public class Greeting { public string GetMessage() => \"Hello\"; } }");
        WriteFile("Program.cs", "using Demo; class App { static void Main() { var g = new Greeting(); } }");

        _server = new McpTestServer(_tempDir);
    ...
    {
        return obj?.GetValue(key, StringComparison.OrdinalIgnoreCase)?.Value<string>();
    }

    private static int Int(JObject? obj, string key, int def = 0)
    {
        return obj?.GetValue(key, StringComparison.OrdinalIgnoreCase)?.Value<int>() ?? def;
    }
    ...
    public async Task Update_FirstTime_FallsBackToFullBuild()
    {
        ...
        try
        {
            using var freshServer = new McpTestServer(freshDir);
