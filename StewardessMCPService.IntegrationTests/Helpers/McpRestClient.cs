...
public sealed class McpRestClient
{
    ...
    {
        var (data, isError) =
            await CallToolAsync("create_directory", new { path = relativePath, create_parents = true });
        if (isError)
            return new EditResult
            {
                Success = false,
                ErrorMessage = data.GetValue("message", StringComparison.OrdinalIgnoreCase)?.Value<string>() ??
    ...
    {
        var (data, isError) = await CallToolAsync("write_file", new { path = relativePath, content });
        if (isError)
            return new EditResult
            {
                Success = false,
    ...
    {
        return PostAsync<RunBuildRequest, CommandResult>(
            "api/command/build",
            new RunBuildRequest
            {
    ...
    {
        return PostAsync<RunCustomCommandRequest, CommandResult>(
            "api/command/run",
