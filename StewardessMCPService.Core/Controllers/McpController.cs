...
public sealed class McpController : BaseController
{
        ...
        [FromBody] JObject body, CancellationToken ct)
    {
        if (body == null)
        {
            var errResp = McpResponse.Err(null!, McpErrorCodes.ParseError,
    ...
    public IActionResult ListTools()
    {
        ...
        return Ok(new McpListToolsResult { Tools = new List<McpToolDefinition>(tools) });
    }
    ...
    public async Task<IActionResult> GetManifest(CancellationToken ct)
    {
        ...
        var manifest = new McpCapabilitiesManifest
        {
            ...
            {
                RepositoryName = Path.GetFileName(Settings.RepositoryRoot.TrimEnd('\\', '/')),
                // Do not expose the full server-side absolute path; return only the leaf name.
                RepositoryRoot = Path.GetFileName(Settings.RepositoryRoot.TrimEnd('\\', '/')),
                IsGitRepository = isGit,
                CurrentBranch = branch!
            },

            Tools = new List<McpToolDefinition>(tools)
        };
