...
public sealed class McpStreamableController : BaseController
{
    ...
    private static readonly JsonSerializerSettings SseJsonSettings = new()
    {
        ...
        CancellationToken ct)
    {
        ...
        var progressToken = ExtractProgressToken(request);

        // ── SSE streaming mode ────────────────────────────────────────────────
        var wantsStream = Request.GetTypedHeaders().Accept?.Count > 0 &&
                          Request.Headers["Accept"].ToString()
                              .Contains(SseContentType, StringComparison.OrdinalIgnoreCase);
    ...
    public async Task GetNotifications(CancellationToken ct)
    {
        ...
        {
            /* client disconnected or session terminated */
        }
        finally
        {
    ...
    {
        return JsonConvert.SerializeObject(response, SseJsonSettings);
    }
    ...
    {
        return SerializeResponse(McpResponse.Err(id!, code, message));
    }
}