...
public sealed class RepositoriesController : ControllerBase
{
    ...
    private ApiRepository BuildRepoInfo()
    {
        ...
        return new ApiRepository
        {
            ...
            CreatedAt = dirInfo.Exists ? new DateTimeOffset(dirInfo.CreationTimeUtc) : null,
            UpdatedAt = dirInfo.Exists ? new DateTimeOffset(dirInfo.LastWriteTimeUtc) : null
        };
    }
    ...
    public async Task<IActionResult> GetFile(string repoId, string filePath, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var absPath = Path.Combine(Settings.RepositoryRoot,
                filePath.Replace('/', Path.DirectorySeparatorChar));
            var fileInfo = new FileInfo(absPath);
            var lastModified = fileInfo.Exists
    ...
    public IActionResult ListPullRequests(string repoId)
    {
        return StatusCode(501,
            ...
            {
                message =
                    "Pull requests require a hosted git service (GitHub/GitLab/Bitbucket) and are not supported by this local repository tool."
            });
    }
