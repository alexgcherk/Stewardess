    ...
    public sealed class CodeManagementController : ControllerBase
    {
        ...
        public async Task<IActionResult> WriteFile([FromBody] CmWriteFileRequest req, CancellationToken ct)
        {
            ...
            try
            {
                ...
                return Ok(new CmWriteFileResponse
                {
                    ...
                    BytesWritten = (long)Encoding.UTF8.GetByteCount(req.Content ?? ""),
        ...
        public async Task<IActionResult> ApplyPatch([FromBody] CmApplyPatchRequest req, CancellationToken ct)
        {
            ...
            try
            {
                ...
                bool allOrNothing = req.TransactionMode == "allOrNothing";

                foreach (var edit in req.Edits)
                {
                    try
                    {
                        EditResult result = edit.PatchType switch
                        {
                ...
                bool applied = fileResults.Count > 0 && fileResults.All(f => f.Changed);
        ...
        public async Task<IActionResult> GetDependencies([FromBody] CmDependenciesRequest req, CancellationToken ct)
        {
            ...
            try
            {
                ...
                {
                    new CmDependencyNode { Id = req.Path, Label = req.Path, Kind = "file", Path = req.Path }
                };
        ...
        public async Task<IActionResult> BuildWorkspace([FromBody] CmBuildRequest req, CancellationToken ct)
        {
            ...
            try
            {
                ...
                var result = await _cmd.RunBuildAsync(new RunBuildRequest
                {
                    ...
                    Arguments = args.Trim(),
                    TimeoutSeconds = req.TimeoutSeconds > 0 ? (int?)req.TimeoutSeconds : null
                }, ct);
        ...
        private static string MapLineEnding(string lineEnding) => lineEnding switch
        {
            "LF" => "lf",
            "CRLF" => "crlf",
            "CR" => "cr",
        ...
        private IActionResult HandleException(Exception ex) => ex switch
        {
            ...
            System.IO.FileNotFoundException _ or System.IO.DirectoryNotFoundException _ =>
