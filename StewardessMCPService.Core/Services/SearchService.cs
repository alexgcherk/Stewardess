...
public sealed class SearchService : ISearchService
{
    ...
    /// <inheritdoc />
    public Task<FileNameSearchResponse> SearchFileNamesAsync(SearchFileNamesRequest request,
        CancellationToken ct = default)
    {
        ...
        var isRegex = PatternHelper.IsLikelyRegex(pattern);
        var isWildcard = !isRegex && (pattern.Contains('*') || pattern.Contains('?'));
        ...
        var truncated = false;
        var maxResults = Math.Min(request!.MaxResults, _settings.MaxSearchResults);
    ...
    /// <inheritdoc />
    public Task<FileNameSearchResponse> SearchByExtensionAsync(SearchByExtensionRequest request,
        CancellationToken ct = default)
    {
        var exts = new HashSet<string>(
            (request.Extensions ?? new List<string>()).Select(e =>
                e.StartsWith(".") ? e.ToLowerInvariant() : "." + e.ToLowerInvariant()),
    ...
    public Task<SearchResponse> SearchSymbolAsync(SearchSymbolRequest request, CancellationToken ct = default)
    {
        ...
        var kind = (request.SymbolKind ?? "").ToLowerInvariant();
        ...
        CancellationToken ct)
    {
        ...
        foreach (var filePath in EnumerateFiles(searchRoot, "*.*", ct))
        {
            ...
            try
            {
                lines = await Task.Run(() => File.ReadAllLines(filePath), ct).ConfigureAwait(false);
            }
            catch
            {
                continue;
    ...
    private static List<string> GetContextLines(string[] lines, int currentIdx, int offset, int count)
    {
        ...
        var start = offset < 0 ? currentIdx + offset : currentIdx + offset;
