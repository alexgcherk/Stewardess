...
public sealed class EditService : IEditService
{
    ...
    private static readonly ConcurrentDictionary<string, RollbackEntry> _rollbackRegistry = new(StringComparer.Ordinal);
    ...
    public async Task<EditResult> MovePathAsync(MovePathRequest request, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.DestinationPath))
            throw new ArgumentException("DestinationPath is required.", nameof(request));
    ...
    public async Task<EditResult> ReplaceLinesAsync(ReplaceLinesRequest request, CancellationToken ct = default)
    {
        ...
        if (request.EndLine < request.StartLine)
            throw new ArgumentException("EndLine must be >= StartLine.", nameof(request));
    ...
    public async Task<EditResult> PatchFileAsync(PatchFileRequest request, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Patch))
            throw new ArgumentException("Patch is required.", nameof(request));
    ...
    public async Task<BatchEditResult> ApplyBatchEditsAsync(BatchEditRequest request, CancellationToken ct = default)
    {
        if (request == null) throw new ArgumentNullException(nameof(request));
        if (request.Edits == null || request.Edits.Count == 0)
            throw new ArgumentException("Edits list must not be empty.", nameof(request));
            ...
            catch (Exception ex)
            {
                ...
                if (!dryRun && rollbacks.Count > 0)
                    foreach (var tok in rollbacks)
                        try
                        {
                            await RollbackAsync(new RollbackRequest { RollbackToken = tok }, ct);
                        }
                        catch
                        {
                            /* best-effort */
    ...
    {
        if (request == null) throw new ArgumentNullException(nameof(request));
        if (string.IsNullOrWhiteSpace(request.RollbackToken))
            throw new ArgumentException("RollbackToken is required.", nameof(request));
        ...
        await Task.Run(() =>
        {
            ...
            File.Copy(entry.BackupAbsolutePath, targetAbs, true);
        }, ct);

        await _auditService.LogOperationAsync(
            Guid.NewGuid().ToString("N"),
            null,
            AuditOperationType.Rollback,
            "rollback",
            "service",
            entry.TargetRelativePath,
            AuditOutcome.Success,
            null,
            $"Rolled back from backup: {entry.BackupAbsolutePath}",
    ...
    /// <inheritdoc />
    public async Task<PreviewChangesResponse> PreviewChangesAsync(PreviewChangesRequest request,
    ...
    private (string backupRelPath, string rollbackToken) CreateBackup(string absPath)
    {
        ...
        File.Copy(absPath, backupAbs, true);
    ...
    internal static string GenerateUnifiedDiff(string originalContent, string newContent, string relPath)
    {
        ...
        return FormatUnifiedDiff(edits, origLines, newLines, relPath, 3);
    }
    ...
    private static List<(char Op, int OrigIdx, int NewIdx)> BuildEditScript(string[] a, string[] b)
    {
        ...
        if (m > MaxLinesForLcs || n > MaxLinesForLcs)
        {
            ...
            for (var i = 0; i < m; i++) fb.Add(('-', i, -1));
            for (var j = 0; j < n; j++) fb.Add(('+', -1, j));
            return fb;
        }
        ...
        string relPath, int contextLines)
    {
        ...
        for (var i = 1; i < changePositions.Count; i++)
        ...
        {
            var hunkFrom = Math.Max(0, rangeStart - contextLines);
            var hunkTo = Math.Min(edits.Count - 1, rangeEnd + contextLines);

            int origLine1 = -1, newLine1 = -1, origCount = 0, newCount = 0;
            var body = new List<string>();

            for (var k = hunkFrom; k <= hunkTo; k++)
            {
    ...
    internal static string ApplyPatch(string fileContent, string patchText, int fuzzFactor)
    {
        ...
        var offset = 0; // accumulates line-count delta as hunks are applied

        foreach (var hunk in hunks)
        {
            var target = hunk.OldStart - 1 + offset; // 0-based
            var applied = false;

            for (var fuzz = 0; fuzz <= fuzzFactor && !applied; fuzz++)
            {
                ...
                foreach (var start in candidates)
                {
                    ...
                    var added = hunk.Lines.Count(l => l.Op == '+');
                    var removed = hunk.Lines.Count(l => l.Op == '-');
    ...
    private static void ApplyHunk(List<string> lines, PatchHunk hunk, int startLine)
    {
        ...
        var removeCount = hunk.Lines.Count(l => l.Op == ' ' || l.Op == '-');
        var actualRemove = Math.Min(removeCount, lines.Count - startLine);
        ...
        StringComparison cmp, int maxReplacements)
    {
        ...
        {
            var pos = content.IndexOf(oldText, index, cmp);
            if (pos < 0) break;
    ...
    {
        return Task.Run(() => File.ReadAllText(absPath, Encoding.UTF8), ct);
    }
    ...
    {
        return _auditService.LogOperationAsync(
            Guid.NewGuid().ToString("N"),
            opts?.SessionId,
            MapAuditType(opName),
            opName,
