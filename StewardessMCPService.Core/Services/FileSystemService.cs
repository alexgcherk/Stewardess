...
public sealed class FileSystemService : IFileSystemService
{
    ...
    private static readonly HashSet<string> _knownBinaryExtensions = new(
    ...
    private static readonly Regex _nsRegex = new(@"^\s*namespace\s+([\w.]+)", RegexOptions.Compiled);

    private static readonly Regex _typeRegex =
        new(
            @"^\s*(public|internal|protected|private|sealed|abstract|static|partial).*?(class|interface|struct|enum)\s+(\w+)",
            RegexOptions.Compiled);

    private static readonly Regex _methodRegex =
        new(
            @"^\s*(public|internal|protected|private|static|virtual|override|async|sealed|abstract)\s+[\w<>\[\],\s]+\s+(\w+)\s*\(",
            RegexOptions.Compiled);

    private static readonly Regex _usingRegex = new(@"^\s*using\s+([\w.]+)\s*;", RegexOptions.Compiled);
    ...
    public Task<ListDirectoryResponse> ListDirectoryAsync(ListDirectoryRequest request, CancellationToken ct = default)
    {
        ...
        {
            var isBlocked = _pathValidator.IsFolderBlocked(item.FullName)
        ...
        var truncated = sorted.Count > _settings.MaxDirectoryEntries;
    ...
    /// <inheritdoc />
    public async Task<ReadMediaFileResponse> ReadMediaFileAsync(ReadMediaFileRequest request,
    ...
    /// <inheritdoc />
    public async Task<ReadFileRangeResponse> ReadFileRangeAsync(ReadFileRangeRequest request,
        CancellationToken ct = default)
    {
        var validation = _pathValidator.ValidateRead(request?.Path, out var absPath);
        if (!validation.IsValid)
        ...
        var end = request.EndLine < 1 ? total : Math.Min(request.EndLine, total);
    ...
    /// <inheritdoc />
    public async Task<ReadMultipleFilesResponse> ReadMultipleFilesAsync(ReadMultipleFilesRequest request,
        CancellationToken ct = default)
    {
                   ...
                   81920, true))
        {
            ...
            switch (algorithm)
            {
                case "MD5":
                    ...
                    {
                        hashBytes = await Task.Run(() => md5.ComputeHash(stream), ct);
                    }
                ...
                case "SHA1":
                    ...
                    {
                        hashBytes = await Task.Run(() => sha1.ComputeHash(stream), ct);
                    }
                ...
                default:
                    ...
                    {
                        hashBytes = await Task.Run(() => sha.ComputeHash(stream), ct);
                    }
        ...
        ListTreeRequest request, ref int totalFiles, ref int totalDirs, ref bool truncated)
    {
        ...
        var blocked = _settings.BlockedFolders.Contains(info.Name);
        ...
        foreach (var item in items)
            ...
            {
                var childBlocked = _settings.BlockedFolders.Contains(di.Name);
    ...
    private RepositoryPolicyInfo BuildPolicyInfo()
    {
        return new RepositoryPolicyInfo
        {
