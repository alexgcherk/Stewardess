...
public sealed class CommandService : ICommandService
{
        ...
        CancellationToken ct = default)
    {
        ...
        _ = _auditService.LogOperationAsync(
            ...
            null,
            isBuild ? AuditOperationType.RunBuild
            : isTest ? AuditOperationType.RunTests
            : AuditOperationType.RunCommand,
            isBuild ? "run_build" : isTest ? "run_tests" : "run_command",
            "internal",
            workDir,
            exitCode == 0 ? AuditOutcome.Success : AuditOutcome.Failure,
            exitCode != 0 ? "EXIT_" + exitCode : null,
            fullCommand,
    ...
    private string ResolveWorkingDirectory(string relativeWorkDir)
    {
        ...
        if (!_pathValidator.Validate(relativeWorkDir, out var absPath))
            throw new UnauthorizedAccessException(
                $"Working directory is outside the repository root: {relativeWorkDir}");
