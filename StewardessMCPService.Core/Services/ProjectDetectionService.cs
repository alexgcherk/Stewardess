...
public sealed class ProjectDetectionService : IProjectDetectionService
{
    ...
    private static readonly Regex _slnProjectRegex = new(
        @"Project\(""\{[^}]+\}""\)\s*=\s*""([^""]+)""\s*,\s*""([^""]+)""",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);

    private static readonly Regex _targetFrameworkRegex = new(
        @"<TargetFramework(?:Version|)>\s*([^<]+)\s*</TargetFramework",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);

    private static readonly Regex _outputTypeRegex = new(
    ...
    private static bool TryHasConnectionStrings(string absPath)
    {
        try
        {
            return File.ReadAllText(absPath).IndexOf("<connectionStrings", StringComparison.OrdinalIgnoreCase) >= 0;
        }
        catch
        {
            return false;
