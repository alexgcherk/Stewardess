...
public sealed class PathValidator
{
    ...
    public PathValidator(McpServiceSettings settings)
    {
        ...
        RepositoryRoot = EnsureTrailingSeparator(Path.GetFullPath(settings.RepositoryRoot));
    ...
    public ValidationResult Validate(string? relativePath, out string absolutePath)
    {
        ...
        try
        {
            candidate = Path.GetFullPath(Path.Combine(RepositoryRoot, sanitised));
        }
        catch (Exception ex)
        {
    ...
    public ValidationResult ValidateRead(string? relativePath, out string absolutePath)
    {
        ...
        return CheckFolderAndExtension(absolutePath, false);
    }
    ...
    public ValidationResult ValidateWrite(string? relativePath, out string absolutePath)
    {
        ...
        return CheckFolderAndExtension(absolutePath, true);
    }
    ...
    public string ToRelativePath(string absolutePath)
    {
        ...
        if (normalised.Length < RepositoryRoot.Length)
            return string.Empty;

        var rel = normalised.Substring(RepositoryRoot.Length);
    ...
    public string ToAbsolutePath(string relativePath)
    {
        if (string.IsNullOrWhiteSpace(relativePath))
            return RepositoryRoot.TrimEnd(Path.DirectorySeparatorChar);

        var sanitised = relativePath.TrimStart('/', '\\').Replace('/', Path.DirectorySeparatorChar);
        return Path.GetFullPath(Path.Combine(RepositoryRoot, sanitised));
    }
    ...
    private bool IsInsideRoot(string absolutePath)
    {
        ...
        return absolutePath.StartsWith(RepositoryRoot, StringComparison.OrdinalIgnoreCase)
               || string.Equals(
...
public sealed class ValidationResult
{
    ...
    {
        return new ValidationResult { IsValid = false, ErrorCode = code, ErrorMessage = message };
    }
