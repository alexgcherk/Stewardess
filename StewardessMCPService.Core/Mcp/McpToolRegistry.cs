...
using McpError = StewardessMCPService.CodeIndexing.Query.McpError;
...
public sealed class McpToolRegistry
{
    ...
    {
        return _tools.Values.Select(e => e.Definition).ToList();
    }
    ...
    private void RegisterRepositoryTools()
    {
        Add("get_repository_info",
            "repository",
            "Returns high-level information about the repository: name, root, file counts, and allowed policy.",
            Schema(),
            async (args, ct) =>
            {
        ...
        Add("list_directory",
            "repository",
            "Lists immediate children (files and sub-directories) of the given directory.",
            Schema(
                Prop("path", "string", "Relative path to the directory (empty = repo root)."),
                Prop("include_blocked", "boolean", "Include blocked folders in the listing (default false).",
                    def: false),
                Prop("sort_by", "string", "Sort order: 'name' (default) or 'size' (largest files first).", def: "name",
                    enums: new[] { "name", "size" })),
            async (args, ct) =>
            {
                ...
                {
                    Path = Str(args, "path", ""),
                    IncludeBlocked = Bool(args, "include_blocked"),
        ...
        Add("list_directory_with_sizes",
            "repository",
            "Lists immediate children of a directory with file sizes, sortable by name or size. " +
            ...
            Schema(
                Prop("path", "string", "Relative path to the directory (empty = repo root)."),
                Prop("sort_by", "string", "Sort order: 'name' (alphabetical, default) or 'size' (largest files first).",
                    def: "name", enums: new[] { "name", "size" })),
            async (args, ct) =>
            {
                var req = new ListDirectoryRequest
                {
        ...
        Add("path_exists",
            "repository",
            "Checks whether a path (file or directory) exists in the repository.",
            Schema(
                Prop("path", "string", "Relative path to check.", true)),
            async (args, ct) =>
            {
        ...
        Add("get_metadata",
            "repository",
            "Returns detailed metadata (size, dates, encoding, line count) for a file or directory.",
            Schema(
                Prop("path", "string", "Relative path of the file or directory.", true)),
            async (args, ct) =>
            {
        ...
        Add("detect_encoding",
            "repository",
            "Detects the character encoding of a file (UTF-8, UTF-16, ASCII, etc.).",
            Schema(
    ...
    private void RegisterSearchTools()
    {
        Add("search_text",
            "search",
            "Searches for a literal text string across repository files. Returns file paths, line numbers, and match excerpts.",
            Schema(
                Prop("query", "string", "Literal text to search for.", true),
            ...
            async (args, ct) =>
            {
                var req = new SearchTextRequest
                {
                    ...
                    WholeWord = Bool(args, "whole_word"),
        ...
        Add("search_regex",
            "search",
            "Searches using a .NET regular expression pattern.",
            Schema(
                Prop("pattern", "string", ".NET regex pattern.", true),
        ...
        Add("search_file_names",
            "search",
            "Finds files whose names match a substring, wildcard, or regex pattern.",
            Schema(
                Prop("pattern", "string",
                    "Filename pattern: substring, glob (*, ?), or regex (e.g. '^Foo\\.cs$'). Regex is auto-detected.",
                    true),
                Prop("search_path", "string", "Restrict to subdirectory."),
                Prop("max_results", "integer", "Max results (default 100).", def: 100),
                Prop("page", "integer", "Page number (1-based, default 1).", def: 1),
        ...
        Add("search_by_extension",
            "search",
            "Returns all files matching one or more file extensions.",
            Schema(
                Prop("extensions", "array", "Extensions to find, e.g. [\".cs\",\".js\"].", true),
                Prop("search_path", "string", "Restrict to subdirectory."),
        ...
        Add("search_symbol",
            ...
            Schema(
                Prop("symbol_name", "string", "Symbol name to search for.", true),
                Prop("symbol_kind", "string", "Optional symbol kind filter.",
                    enums: new[]
                    {
                        "class", "interface", "method", "property", "enum", "field", "constructor", "event", "delegate",
                        "namespace"
                    }),
                Prop("search_path", "string", "Restrict to subdirectory."),
        ...
        Add("find_references",
            "search",
            "Best-effort reference search — finds textual usages of an identifier across all files.",
            Schema(
                Prop("identifier", "string", "Identifier to look for.", true),
                Prop("search_path", "string", "Restrict to subdirectory."),
                Prop("extensions", "array", "File extensions."),
                Prop("max_results", "integer", "Max results (default 100).", def: 100)),
            async (args, ct) =>
    ...
    private void RegisterReadTools()
    {
        Add("read_file",
            "files",
            "Reads the full content of a file. Large files are automatically truncated at the server limit. Use 'head' or 'tail' to limit to the first or last N lines.",
            Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("max_bytes", "integer", "Maximum bytes to return (server limit applies)."),
                Prop("return_base64", "boolean", "Return base64-encoded content instead of text (for binary files).",
                    def: false),
            ...
            async (args, ct) =>
            {
                var req = new ReadFileRequest
                {
                    ...
                    ReturnBase64 = Bool(args, "return_base64"),
        ...
        Add("read_file_range",
            ...
            Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("start_line", "integer", "First line (1-based, inclusive).", true),
                Prop("end_line", "integer", "Last line (1-based, inclusive).", true)),
            async (args, ct) =>
            {
        ...
        Add("read_multiple_files",
            "files",
            "Reads multiple files in a single call. Returns contents and any per-file errors.",
            Schema(
                Prop("paths", "array", "Array of relative file paths to read.", true)),
            async (args, ct) =>
            {
        ...
        Add("read_media_file",
            "files",
            "Reads an image, audio, or other binary file and returns it as base64-encoded content with its detected MIME type. " +
        ...
        Add("get_file_hash",
            "files",
            "Computes a hash digest (SHA-256 by default) of a file's contents.",
            Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("algorithm", "string", "Hash algorithm: SHA256 (default), MD5, SHA1.",
                    enums: new[] { "MD5", "SHA1", "SHA256" })),
            async (args, ct) =>
            {
        ...
        Add("get_file_structure",
            "code-intelligence",
            "Returns a structural summary of a code file: namespaces, types, and methods (parsed via text heuristics).",
            Schema(
                Prop("path", "string", "Relative file path.", true)),
            async (args, ct) =>
            {
    ...
    {
        var ro = _settings.ReadOnlyMode;

        Add("write_file",
            "edit",
            "Overwrites (or creates) a file with new content. Supports dry-run and backup.",
            isDestructive: true,
            isDisabled: ro,
            disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("content", "string", "New full content of the file.", true),
                Prop("encoding", "string", "Encoding: utf-8 (default), utf-8-bom, utf-16, ascii.",
                    enums: new[] { "utf-8", "utf-8-bom", "utf-16", "ascii" }),
        ...
        Add("create_file",
            "edit",
            "Creates a new file. Fails by default if the file already exists.",
        ...
        Add("create_directory",
            "edit",
            "Creates a directory and optionally all parent directories.",
            isDestructive: false, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("path", "string", "Relative directory path.", true),
        ...
        Add("rename_path",
            "edit",
            "Renames a file or directory in place (new name, same parent directory).",
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("path", "string", "Relative path of the item to rename.", true),
                Prop("new_name", "string", "New name (filename only, no path separator).", true),
                Prop("dry_run", "boolean", "Simulate (default false).", def: false)),
            handler: async (args, ct) =>
            {
        ...
        Add("move_path",
            ...
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("source_path", "string", "Relative source path.", true),
                Prop("destination_path", "string", "Relative destination path.", true),
        ...
        Add("delete_file",
            "edit",
            "Deletes a file. Creates a backup first by default (rollback is possible).",
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("path", "string", "Relative file path.", true),
        ...
        Add("delete_directory",
            "edit",
            "Deletes a directory. Requires recursive=true to delete non-empty directories.",
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("path", "string", "Relative directory path.", true),
        ...
        Add("append_file",
            ...
            schema: Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("content", "string", "Content to append.", true),
                Prop("dry_run", "boolean", "Simulate (default false).", def: false)),
            handler: async (args, ct) =>
            {
        ...
        Add("replace_text",
            "edit",
            "Replaces all (or a limited number of) occurrences of a literal string in a file.",
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("old_text", "string", "Text to replace.", true),
            ...
            handler: async (args, ct) =>
            {
                var req = new ReplaceTextRequest
                {
                    ...
                    MaxReplacements = Int(args, "max_replacements"),
        ...
        Add("edit_file",
            ...
            schema: Schema(
                ...
                Prop("change_reason", "string", "Agent-supplied reason for the change (audit log)."),
                PropWithItems("edits", "Ordered list of text edits to apply.", true,
                    new
                    {
                        type = "object",
                        properties = new
            ...
            handler: async (args, ct) =>
            {
                ...
                var editsList = editsRaw is JArray ja
                    ? ja.ToObject<List<JObject>>()
                    : new List<JObject>();
        ...
        Add("replace_lines",
            ...
            schema: Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("start_line", "integer", "First line to replace (1-based, inclusive).", true),
                Prop("end_line", "integer", "Last line to replace (1-based, inclusive).", true),
                Prop("new_content", "string", "Replacement content.", true),
                Prop("dry_run", "boolean", "Simulate (default false).", def: false)),
            handler: async (args, ct) =>
            {
        ...
        Add("patch_file",
            ...
            schema: Schema(
                Prop("path", "string", "Relative file path.", true),
                Prop("patch", "string", "Unified diff patch text.", true),
        ...
        Add("apply_diff",
            "edit",
            "Applies a multi-file unified diff in a single atomic operation.",
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("diff", "string", "Full multi-file unified diff text.", true),
                Prop("dry_run", "boolean", "Simulate (default false).", def: false)),
            handler: async (args, ct) =>
            {
        ...
        Add("batch_edit",
            "edit",
            "Executes multiple heterogeneous edits atomically. Rolls back all changes if any operation fails.",
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                PropWithItems("edits", "Array of edit items. Each item must have 'operation' and 'arguments'.", true,
                    new
                    {
                        type = "object",
                        properties = new
                        {
                            ...
                            arguments = new
                            {
                                type = "object",
                                description =
                                    "Operation-specific arguments (same as the corresponding single-file tool)."
                            }
                        },
        ...
        Add("preview_changes",
            "edit",
            "Dry-runs a batch of edits and returns a preview diff plus an approval token.",
            isDestructive: false, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("proposed_edits", "object", "BatchEditRequest containing the edits to preview.", true)),
            handler: async (args, ct) =>
            {
        ...
        Add("rollback",
            ...
            isDestructive: true, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("rollback_token", "string", "Token returned by a previous write/delete operation.", true)),
            handler: async (args, ct) =>
            {
    ...
    private void RegisterGitTools()
    {
        Add("get_git_status",
            "git",
            "Returns the current git status: branch, HEAD commit, and per-file staged/unstaged/untracked changes.",
            Schema(
        ...
        Add("get_git_diff",
            "git",
            "Returns the unified diff for working-tree or staged changes. Scope: unstaged (default), staged, head, or commit:sha1..sha2.",
            Schema(
                ...
                Prop("scope", "string", "Diff scope: unstaged (default), staged, head.", def: "unstaged",
                    enums: new[] { "unstaged", "staged", "head" }),
        ...
        Add("get_commit",
            "git",
            "Returns the full details of a single commit by SHA: author, committer, message, changed files, and optionally the diff patch.",
            Schema(
                Prop("sha", "string", "Full or abbreviated commit SHA (minimum 4 hex chars).", true),
                Prop("include_diff", "boolean", "Include the unified diff patch in the response (default: true).",
                    def: true)),
            async (args, ct) =>
            {
    ...
    private void RegisterCommandTools()
    {
        ...
        Add("run_build",
            "command",
            "Runs the configured build command (dotnet build or msbuild). Only allowed commands are accepted.",
            isDestructive: false, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
        ...
        Add("run_tests",
            "command",
            "Runs the configured test command (dotnet test). Only allowed commands are accepted.",
            isDestructive: false, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
        ...
        Add("run_command",
            "command",
            $"Runs a custom command from the AllowedCommands whitelist. Allowed prefixes: {allowedLabel}.",
            isDestructive: false, isDisabled: ro, disabledReason: ro ? "Read-only mode" : null,
            schema: Schema(
                Prop("command", "string", "Full command line. Must start with an allowed prefix.", true),
                Prop("working_directory", "string", "Working directory relative to repo root."),
                Prop("timeout_seconds", "integer", "Timeout override in seconds.")),
            handler: async (args, ct) =>
            {
    ...
    private void RegisterCodeIndexTools()
    {
        ...
        Add("code_index.build",
            "code_index",
            "Builds or rebuilds the structural code index for the given repository root. " +
            "Enumerates all eligible files, detects languages, and parses declarations. " +
            "Returns the resulting snapshot ID and statistics.",
            Schema(
                Prop("root_path", "string", "Absolute path to the repository root to index.", true),
                Prop("parse_mode", "string", "Depth of parsing.", def: "Declarations",
                    enums: new[] { "OutlineOnly", "Declarations", "DeclarationsAndReferences" }),
                Prop("force_rebuild", "boolean", "Discard existing index and rebuild from scratch (default false).",
                    def: false)),
            async (args, ct) =>
            {
                var modeStr = Str(args, "parse_mode", "Declarations");
                var mode = Enum.TryParse<ParseMode>(modeStr, true, out var m)
                    ? m
                    : ParseMode.Declarations;

                var req = new IndexBuildRequest
                {
                    RootPath = Str(args, "root_path"),
                    ParseMode = mode,
                    ForceRebuild = Bool(args, "force_rebuild")
                };
        ...
        Add("code_index.get_status",
            "code_index",
            "[Deprecated: use code_index.get_index_status instead] Returns the current indexing state for a repository root.",
            Schema(
                Prop("root_path", "string", "Absolute repository root path.", true)),
            async (args, ct) =>
            {
        ...
        Add("code_index.list_files",
            "code_index",
            "Returns a paginated list of indexed files with their language, parse status, and node counts. " +
            ...
            async (args, ct) =>
            {
                var lang = Str(args, "language");
                var req = new ListFilesRequest
                {
                    SnapshotId = Str(args, "snapshot_id"),
                    RootPath = Str(args, "root_path"),
        ...
        Add("code_index.get_file_outline",
            "code_index",
            "Returns the structural outline of a single file: namespaces, classes, methods, functions, and other declarations. " +
            "Include source line spans for each node.",
            Schema(
                Prop("file_path", "string", "Repository-relative path to the file.", true),
            ...
            async (args, ct) =>
            {
                var maxDepth = Int(args, "max_depth");
                var req = new GetFileOutlineRequest
                {
                    FilePath = Str(args, "file_path"),
                    SnapshotId = Str(args, "snapshot_id"),
                    MaxDepth = maxDepth == 0 ? null : maxDepth,
                    IncludeSourceSpans = Bool(args, "include_spans", true),
                    IncludeConfidence = Bool(args, "include_confidence")
                };
                var result = await query.GetFileOutlineAsync(req, ct).ConfigureAwait(false);
                if (result == null)
                    return ErrorResult(McpErrorCode.FileNotFound, "File not found in index",
                        new { file_path = req.FilePath });
                return ToResult(result);
            });

        Add("code_index.get_snapshot_info",
            "code_index",
            "Returns metadata for the latest snapshot of a repository root: file counts, language breakdown, and build timestamps.",
            Schema(
            ...
            async (args, ct) =>
            {
                var result = await query.GetSnapshotInfoAsync(
                    ...
                    ct).ConfigureAwait(false);
                if (result == null)
                    return ErrorResult(McpErrorCode.SnapshotNotFound, "No snapshot found for repository",
                        new { root_path = Str(args, "root_path") });
                return ToResult(result);
            });

        Add("code_index.list_roots",
            "code_index",
            "[Deprecated: use code_index.list_repositories instead] Returns the list of repository root paths that have a published code index snapshot.",
            Schema(),
            async (args, ct) =>
            {
        ...
        Add("code_index.get_language_capabilities",
            "code_index",
            "Returns the list of supported language IDs and their parser adapter capabilities.",
            Schema(),
            (args, ct) =>
            {
                var adapters = new IParserAdapter[]
                {
        ...
        Add("code_index.find_symbols",
            ...
            Schema(
                Prop("query_text", "string", "Text to match against symbol names or qualified names.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query (overrides root_path)."),
                Prop("root_path", "string", "Repository root path; used to resolve the latest snapshot."),
                ...
                Prop("include_occurrence_count", "boolean", "Include occurrence counts per symbol (default true).",
                    def: true),
                Prop("include_members_summary", "boolean", "Include member summary for type symbols (default false).",
                    def: false),
                Prop("page", "integer", "Page number (1-based, default 1).", def: 1),
                Prop("page_size", "integer", "Results per page (default 50, max 200).", def: 50)),
            async (args, ct) =>
            {
                var langStr = Str(args, "language");
                var kindStr = Str(args, "kind");
                var containerStr = Str(args, "container");
                var req = new FindSymbolsRequest
                {
                    QueryText = Str(args, "query_text"),
                    SnapshotId = Str(args, "snapshot_id") ?? (Str(args, "root_path") != null ? null : null),
                    MatchMode = Str(args, "match_mode", "prefix"),
                    LanguageFilter = langStr != null ? new[] { langStr } : null,
                    KindFilter = kindStr != null && Enum.TryParse<SymbolKind>(kindStr, true, out var k)
                        ? new[] { k }
                        : null,
                    ContainerFilter = containerStr != null ? new[] { containerStr } : null,
                    IncludeOccurrenceCount = Bool(args, "include_occurrence_count", true),
                    IncludeMembersSummary = Bool(args, "include_members_summary"),
                    Page = Int(args, "page", 1),
                    PageSize = Math.Min(Int(args, "page_size", 50), 200)
                };
        ...
        Add("code_index.get_symbol",
            "code_index",
            "Returns full details for a single logical symbol by its stable symbol ID, " +
            ...
            Schema(
                Prop("symbol_id", "string", "Stable symbol ID to retrieve.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query."),
                Prop("include_primary_occurrence", "boolean", "Include primary declaration location (default true).",
                    def: true),
                Prop("include_members_summary", "boolean", "Include members summary for type symbols (default true).",
                    def: true),
            ...
            async (args, ct) =>
            {
                ...
                if (result?.Symbol == null)
                    return ErrorResult(McpErrorCode.SymbolNotFound, "Symbol not found",
                        new { symbol_id = req.SymbolId });
        ...
        Add("code_index.get_symbol_occurrences",
            "code_index",
            "Returns all occurrences (declarations, references, definitions) of a symbol across the indexed files. " +
            "Each occurrence carries a file path and source line/column.",
            Schema(
                Prop("symbol_id", "string", "Stable symbol ID to look up occurrences for.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query."),
                Prop("role", "string",
                    "Filter to a specific occurrence role: Declaration, Reference, Definition, etc."),
                Prop("page", "integer", "Page number (1-based, default 1).", def: 1),
                Prop("page_size", "integer", "Items per page (default 50, max 200).", def: 50)),
            async (args, ct) =>
            {
                var roleStr = Str(args, "role");
                OccurrenceRole[]? roleFilter = null;
                if (roleStr != null && Enum.TryParse<OccurrenceRole>(roleStr, true, out var role))
                    roleFilter = new[] { role };
                var req = new GetSymbolOccurrencesRequest
                {
                    SymbolId = Str(args, "symbol_id"),
                    SnapshotId = Str(args, "snapshot_id"),
        ...
        Add("code_index.get_symbol_children",
            "code_index",
            "Returns the direct child symbols of a parent symbol (e.g. members of a namespace, nested types in a class). " +
            "Optionally filter by symbol kind.",
            Schema(
                Prop("symbol_id", "string", "Parent symbol ID.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query."),
            ...
            {
                var kindStr = Str(args, "kind");
                var req = new GetSymbolChildrenRequest
                {
                    SymbolId = Str(args, "symbol_id"),
                    SnapshotId = Str(args, "snapshot_id"),
                    KindFilter = kindStr != null && Enum.TryParse<SymbolKind>(kindStr, true, out var k)
        ...
        Add("code_index.get_type_members",
            ...
            async (args, ct) =>
            {
                ...
                if (result == null)
                    return ErrorResult(McpErrorCode.SymbolNotFound, "Symbol not found or not a type",
                        new { symbol_id = req.TypeSymbolId });
                return ToResult(result);
            });

        Add("code_index.resolve_location",
            "code_index",
            "Resolves a symbol ID or occurrence ID to a concrete source location (file path, line, column). " +
            ...
            async (args, ct) =>
            {
                var req = new ResolveLocationRequest
                {
                    SymbolId = Str(args, "symbol_id"),
                    OccurrenceId = Str(args, "occurrence_id"),
                    SnapshotId = Str(args, "snapshot_id")
                };
        ...
        Add("code_index.get_namespace_tree",
            "code_index",
            "Returns a hierarchical tree of namespaces and module containers extracted from the code index. " +
            ...
            async (args, ct) =>
            {
                var langStr = Str(args, "language");
                var maxDepth = Int(args, "max_depth");
                var req = new GetNamespaceTreeRequest
                {
                    SnapshotId = Str(args, "snapshot_id"),
                    LanguageFilter = langStr != null ? new[] { langStr } : null,
                    RootContainer = Str(args, "root_container"),
        ...
        Add("code_index.get_imports",
            "code_index",
            "Returns import, using, and require directives extracted from a specific file. " +
            ...
            async (args, ct) =>
            {
                ...
                if (string.IsNullOrEmpty(filePath))
                    return ErrorResult(McpErrorCode.ValidationError, "file_path is required",
                        new { parameter = "file_path" });
                var req = new GetImportsRequest
                {
                    FilePath = filePath,
                    SnapshotId = Str(args, "snapshot_id"),
                    RootPath = Str(args, "root_path")
        ...
        Add("code_index.get_references",
            "code_index",
            "Returns reference edges for a specific logical symbol. " +
            ...
            Schema(
                Prop("symbol_id", "string", "Symbol ID to query references for.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query."),
            ...
            async (args, ct) =>
            {
                var symbolId = Str(args, "symbol_id");
                if (string.IsNullOrEmpty(symbolId))
                    return ErrorResult(McpErrorCode.ValidationError, "symbol_id is required",
                        new { parameter = "symbol_id" });
                var req = new GetReferencesRequest
                {
                    SymbolId = symbolId,
                    SnapshotId = Str(args, "snapshot_id"),
                    IncludeOutgoing = Bool(args, "include_outgoing", true),
                    IncludeIncoming = Bool(args, "include_incoming")
                };
                var page = Math.Max(1, Int(args, "page", 1));
                var pageSize = Math.Min(200, Math.Max(1, Int(args, "page_size", 50)));
        ...
        Add("code_index.get_file_references",
            "code_index",
            "Returns all reference edges originating from symbols declared in a specific file. " +
            ...
            async (args, ct) =>
            {
                ...
                if (string.IsNullOrEmpty(filePath))
                    return ErrorResult(McpErrorCode.ValidationError, "file_path is required",
                        new { parameter = "file_path" });
                var req = new GetFileReferencesRequest
                {
                    FilePath = filePath,
                    SnapshotId = Str(args, "snapshot_id")
                };
        ...
        Add("code_index.get_dependencies",
            "code_index",
            "Returns outbound hard dependency projections for a logical symbol. " +
            ...
            Schema(
                Prop("symbol_id", "string", "Symbol ID to query dependencies for.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query."),
                ...
                Prop("include_confidence", "boolean", "Include confidence scores in results (default true).",
                    def: true),
                Prop("page", "integer", "Page number (1-based, default 1).", def: 1),
                Prop("page_size", "integer", "Items per page (default 50, max 200).", def: 50)),
            async (args, ct) =>
            {
                ...
                if (string.IsNullOrEmpty(symbolId))
                    return ErrorResult(McpErrorCode.ValidationError, "symbol_id is required",
                        new { parameter = "symbol_id" });
                var req = new GetDependenciesRequest
                {
                    SymbolId = symbolId,
                    SnapshotId = Str(args, "snapshot_id"),
        ...
        Add("code_index.get_dependents",
            "code_index",
            "Returns inbound hard dependent projections for a logical symbol. " +
            "Shows which other symbols in the repository depend on this symbol. " +
            "Use hard_only=false to include weakly-resolved edges.",
            Schema(
                Prop("symbol_id", "string", "Symbol ID to query dependents for.", true),
                Prop("snapshot_id", "string", "Specific snapshot ID to query."),
            ...
            async (args, ct) =>
            {
                ...
                if (string.IsNullOrEmpty(symbolId))
                    return ErrorResult(McpErrorCode.ValidationError, "symbol_id is required",
                        new { parameter = "symbol_id" });
                var req = new GetDependentsRequest
                {
                    SymbolId = symbolId,
                    SnapshotId = Str(args, "snapshot_id"),
                    HardOnly = Bool(args, "hard_only", true),
        ...
        Add("code_index.get_symbol_relationships",
            "code_index",
            "Returns a consolidated relationship payload for a symbol, combining children, " +
            ...
            Schema(
                ...
                Prop("include_dependencies", "boolean", "Include outgoing hard dependency projections (default true).",
                    def: true),
                Prop("include_dependents", "boolean", "Include inbound hard dependent projections (default true).",
                    def: true),
                Prop("include_children", "boolean", "Include direct child symbols (default true).", def: true),
                Prop("max_items_per_section", "integer", "Maximum items to return per section (0 = unlimited).",
                    def: 0),
                Prop("mode", "string", "Response mode.", def: "expanded",
                    enums: new[] { "summary", "expanded" })),
            async (args, ct) =>
            {
                ...
                if (string.IsNullOrEmpty(symbolId))
                    return ErrorResult(McpErrorCode.ValidationError, "symbol_id is required",
                        new { parameter = "symbol_id" });
                var mode = Str(args, "mode", "expanded");
                var maxItems = Int(args, "max_items_per_section");
                if (string.Equals(mode, "summary", StringComparison.OrdinalIgnoreCase))
                    maxItems = 5;
                var req = new GetSymbolRelationshipsRequest
                {
                    SymbolId = symbolId,
                    SnapshotId = Str(args, "snapshot_id"),
        ...
        Add("code_index.get_file_dependencies",
            "code_index",
            "Returns a file-level dependency projection showing which other files this file depends on. " +
            ...
            Schema(
                ...
                Prop("collapse_by_target_file", "boolean",
                    "Collapse all edges to same file into one entry (default true).", def: true)),
            async (args, ct) =>
            {
                var filePath = Str(args, "file_path");
                if (string.IsNullOrEmpty(filePath))
                    return ErrorResult(McpErrorCode.ValidationError, "file_path is required",
                        new { parameter = "file_path" });
                var req = new GetFileDependenciesRequest
                {
                    FilePath = filePath,
                    SnapshotId = Str(args, "snapshot_id"),
                    HardOnly = Bool(args, "hard_only"),
        ...
        Add("code_index.update",
            "code_index",
            "Performs an incremental index update for a repository. " +
            ...
            Schema(
                Prop("root_path", "string", "Absolute repository root path to update (required).", true),
                PropWithItems("changed_files",
                    "Optional array of changed relative file paths. If omitted, change detection runs automatically by comparing file hashes.",
                    items: new { type = "string" })),
            async (args, ct) =>
            {
                var rootPath = Str(args, "root_path");
                if (string.IsNullOrEmpty(rootPath))
                    return ErrorResult(McpErrorCode.ValidationError, "root_path is required",
                        new { parameter = "root_path" });
                ...
                var req = new IndexUpdateRequest
                {
                    RootPath = rootPath,
        ...
        Add("code_index.get_index_status",
            "code_index",
            "Returns the current indexing status for a repository root, including state, latest snapshot, " +
        ...
        Add("code_index.list_repositories",
            "code_index",
            "Lists all repositories that have been indexed, along with their current state, " +
        ...
        Add("code_index.clear_repository",
            "code_index",
            "Removes all stored index state for a repository root. " +
            "The repository will need to be re-indexed before it can be queried. " +
            "Use with caution — this cannot be undone.",
            Schema(
                Prop("root_path", "string", "Absolute repository root path to clear (required).", true)),
            isDestructive: true,
            handler: async (args, ct) =>
            {
                var rootPath = Str(args, "root_path");
        ...
        Add("code_index.ping",
            "code_index",
            "Health check for the code index service. Returns service version, readiness, and available capabilities.",
            Schema(),
            (args, ct) =>
            {
                var adapters = new IParserAdapter[]
    ...
    private void RegisterRepoBrowserTools()
    {
        ...
        Add("repo_browser.print_tree",
            "repo_browser",
            "Returns a flat list of files and directories in the repository. " +
            ...
            Schema(
                ...
                Prop("show_hidden", "boolean", "Include hidden entries (names starting with '.') (default false).",
                    def: false)),
            async (args, ct) =>
            {
                ...
                var showHidden = Bool(args, "show_hidden");
                ...
                void WalkNode(TreeNode node, int depth)
                {
                    ...
                    items.Add(new RepoBrowserTreeItem
                    {
                        ...
                        Depth = depth,
                        HasChildren = isDir ? node.Children != null && node.Children.Count > 0 : null,
                ...
                if (treeResp.Root?.Children != null)
                    foreach (var c in treeResp.Root.Children)
                        WalkNode(c, 0);
        ...
        Add("repo_browser.grep",
            "repo_browser",
            "Searches file contents for symbols, text, imports, or code fragments across the repository. " +
            ...
            Schema(
                Prop("query", "string", "Text or pattern to search for.", true),
            ...
            async (args, ct) =>
            {
                ...
                var globInclude = StrList(args, "glob_include");
                var globExclude = StrList(args, "glob_exclude");
                var caseSens = Bool(args, "case_sensitive");
                ...
                var trunc = sr.Truncated;

                foreach (var file in sr.Files)
                {
                    var rel = file.RelativePath;
                    if (globInclude.Count > 0 && extensions == null &&
                        !globInclude.Any(g => MatchGlob(rel, g))) continue;
                    if (globExclude.Any(g => MatchGlob(rel, g))) continue;

                    var fileHits = 0;
                    foreach (var m in file.Matches)
                    {
                        if (flat.Count >= maxResults)
                        {
                            trunc = true;
                            break;
                        }

                        if (fileHits >= maxPerFile)
                        {
                            trunc = true;
                            break;
                        }
                        ...
                        flat.Add(new RepoBrowserGrepMatch
                        {
                            ...
                            ColumnStart = m.Column > 0 ? m.Column : null,
                            LineText = m.LineText,
        ...
        Add("repo_browser.read_file",
            "repo_browser",
            "Opens a specific file and returns its contents. " +
            ...
            Schema(
                Prop("file_path", "string", "Repository-relative file path.", true),
                Prop("start_line", "integer",
                    "First line to read, inclusive (1-based). Omit to read from the beginning."),
                Prop("end_line", "integer", "Last line to read, inclusive (1-based). -1 = end of file.", def: -1),
                Prop("max_bytes", "integer", "Maximum bytes to return for full-file reads (default 65536).",
                    def: 65536),
                Prop("include_line_numbers", "boolean", "Prepend line numbers to each output line (default true).",
                    def: true)),
            async (args, ct) =>
            {
                var filePath = Str(args, "file_path");
        ...
        Add("repo_browser.find_path",
            "repo_browser",
            "Locates a file or directory when you know its name or part of its path but not its exact location. " +
            ...
            Schema(
                Prop("query", "string", "File name, directory name, or partial path to search for.", true),
        ...
        Add("repo_browser.search",
            "repo_browser",
            "Finds files whose names match a substring, wildcard, or regex pattern (case-insensitive by default). " +
            ...
            Schema(
                Prop("query", "string",
                    "Filename pattern: substring, glob (*, ?), or regex (auto-detected, e.g. '\\.csproj$'). E.g. 'Program', '*.csproj'.",
                    true),
            ...
            async (args, ct) =>
            {
                ...
                var truncated = fileResp.Matches.Count > maxResults || fileResp.Truncated;
    ...
    private void AnnotateAllTools()
    {
        ...
        Annotate("get_repository_info", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    name = new { type = "string" }, rootPath = new { type = "string" },
                    language = new { type = "string" }, framework = new { type = "string" }
                }
            };
        });
        ...
        Annotate("list_directory", d =>
        {
            ...
            {
                UseWhen =
                    "Use to enumerate files/subdirectories at a path. Best for exploring unfamiliar repository structure.",
                ...
                properties = new
                {
                    items = new
                    {
                        ...
                        items = new
                        {
                            ...
                            properties = new
                            {
                                name = new { type = "string" }, path = new { type = "string" },
                                type = new { type = "string", @enum = new[] { "file", "directory" } }
                            }
                        }
                    },
                ...
                properties = new
                {
                    items = new
                    {
                        ...
                        items = new
                        {
                            ...
                            {
                                name = new { type = "string" }, path = new { type = "string" },
                                type = new { type = "string" }, sizeBytes = new { type = "integer" },
                                lastModified = new { type = "string" }
                            }
                        }
                    },
                    totalItems = new { type = "integer" }
        ...
        Annotate("list_tree", d =>
        {
            ...
            {
                UseWhen =
                    "Use to get a recursive directory tree. Useful for understanding project structure at a glance.",
                DoNotUseWhen =
                    "Do not use for very deep trees or large repositories where list_directory is more efficient.",
                TypicalNextTools = new[] { "read_file", "get_file_structure" }
            };
            d.OutputSchema = new
            {
                type = "object",
                properties = new { tree = new { type = "object" }, totalNodes = new { type = "integer" } }
            };
        });
        ...
        Annotate("path_exists", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { exists = new { type = "boolean" }, path = new { type = "string" } }
            };
        });
        ...
        Annotate("get_metadata", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, sizeBytes = new { type = "integer" },
                    lastModified = new { type = "string" }, encoding = new { type = "string" },
                    lineCount = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("search_text", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                DoNotUseWhen =
                    "Do not use for symbol/semantic searches — prefer search_symbol or code_index.find_symbols.",
        ...
        Annotate("search_regex", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                DoNotUseWhen =
                    "Do not use for simple literal string searches — prefer search_text for better performance.",
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    files = new { type = "array" }, totalMatchCount = new { type = "integer" },
                    filesWithMatchesCount = new { type = "integer" }, truncated = new { type = "boolean" },
                    page = new { type = "integer" }, pageSize = new { type = "integer" },
                    totalItems = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        });
        ...
        Annotate("search_file_names", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    matches = new { type = "array" }, totalCount = new { type = "integer" },
                    truncated = new { type = "boolean" }, page = new { type = "integer" },
                    pageSize = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        });
        ...
        Annotate("search_symbol", d =>
        {
            ...
            {
                UseWhen =
                    "Use to find class, method, property, or interface declarations by name using text heuristics.",
                DoNotUseWhen =
                    "Do not use when the code index is available — prefer code_index.find_symbols for semantic accuracy.",
        ...
        Annotate("read_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    ...
                    encoding = new { type = "string" }, lines = new { type = "integer" },
        ...
        Annotate("read_media_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, mimeType = new { type = "string" },
                    base64Content = new { type = "string" }, sizeBytes = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("read_file_range", d =>
        {
            ...
            {
                UseWhen =
                    "Use to read a specific range of lines from a file. More efficient than reading the entire file.",
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, startLine = new { type = "integer" },
                    endLine = new { type = "integer" }, content = new { type = "string" },
                    totalLines = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("get_file_hash", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, algorithm = new { type = "string" }, hash = new { type = "string" }
                }
            };
        });
        ...
        Annotate("get_file_structure", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                DoNotUseWhen =
                    "Do not use for non-code files. For semantic accuracy prefer code_index.get_file_outline.",
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, namespaces = new { type = "array" }, types = new { type = "array" },
                    methods = new { type = "array" }
                }
            };
        });
        ...
        {
            d.SideEffectClass = "file-write";
            d.RiskLevel = "medium";
            d.SupportsDryRun = true;
            d.SupportsRollback = true;
            d.SupportsAuditReason = true;
            d.Tags = new[] { "edit", "destructive" };
            d.UsageGuidance = new McpUsageGuidance
            {
                UseWhen = "Use to overwrite or create a file with new content. Supports dry_run and backup.",
                DoNotUseWhen =
                    "Do not use when you only need to change a few lines — prefer replace_text or replace_lines.",
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, success = new { type = "boolean" },
                    bytesWritten = new { type = "integer" }, backupPath = new { type = "string" }
                }
            };
        });
        ...
        Annotate("create_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { path = new { type = "string" }, success = new { type = "boolean" } }
            };
        });
        ...
        Annotate("create_directory", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { path = new { type = "string" }, created = new { type = "boolean" } }
            };
        });
        ...
        Annotate("rename_path", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    oldPath = new { type = "string" }, newPath = new { type = "string" },
        ...
        Annotate("move_path", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    sourcePath = new { type = "string" }, destinationPath = new { type = "string" },
                    success = new { type = "boolean" }
                }
            };
        });
        ...
        Annotate("append_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                {
                    path = new { type = "string" }, success = new { type = "boolean" },
                    bytesAppended = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("replace_text", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, replacementsApplied = new { type = "integer" },
                    success = new { type = "boolean" }
                }
            };
        });
        ...
        Annotate("edit_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    path = new { type = "string" }, editsApplied = new { type = "integer" },
                    success = new { type = "boolean" }, diff = new { type = "string" }
                }
            };
        });
        ...
        Annotate("replace_lines", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                {
                    path = new { type = "string" }, success = new { type = "boolean" },
                    linesReplaced = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("apply_diff", d =>
        {
            ...
            {
                type = "object",
                properties = new { success = new { type = "boolean" }, filesModified = new { type = "integer" } }
            };
        });
        ...
        Annotate("batch_edit", d =>
        {
            ...
            {
                UseWhen =
                    "Use to execute multiple heterogeneous edits atomically. All-or-nothing with automatic rollback on failure.",
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    success = new { type = "boolean" }, operationsApplied = new { type = "integer" },
                    rollbackToken = new { type = "string" }
                }
            };
        });
        ...
        Annotate("preview_changes", d =>
        {
            ...
            {
                type = "object",
                properties = new { diff = new { type = "string" }, approvalToken = new { type = "string" } }
            };
        });
        ...
        Annotate("get_git_status", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    currentBranch = new { type = "string" }, headCommit = new { type = "string" },
        ...
        Annotate("get_git_diff", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { diff = new { type = "string" }, scope = new { type = "string" } }
            };
        });
        ...
        Annotate("get_git_diff_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { diff = new { type = "string" }, path = new { type = "string" } }
            };
        });
        ...
        Annotate("get_git_log", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { commits = new { type = "array" }, count = new { type = "integer" } }
            };
        });
        ...
        Annotate("get_commit", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    sha = new { type = "string" }, author = new { type = "string" }, message = new { type = "string" },
                    files = new { type = "array" }, diff = new { type = "string" }
                }
            };
        });
        ...
        {
            d.SideEffectClass = "process-execution";
            d.RiskLevel = "medium";
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    exitCode = new { type = "integer" }, output = new { type = "string" },
                    success = new { type = "boolean" }, durationMs = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("run_tests", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                {
                    exitCode = new { type = "integer" }, output = new { type = "string" },
                    passed = new { type = "integer" }, failed = new { type = "integer" },
                    success = new { type = "boolean" }
                }
            };
        });
        ...
        {
            d.SideEffectClass = "service-state-write";
            d.RiskLevel = "low";
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                TypicalNextTools = new[]
                    { "code_index.list_files", "code_index.find_symbols", "code_index.get_snapshot_info" }
            };
            d.OutputSchema = new
            {
                type = "object",
                properties = new
                {
                    snapshotId = new { type = "string" }, fileCount = new { type = "integer" },
                    symbolCount = new { type = "integer" }, durationMs = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("code_index.get_status", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                DoNotUseWhen =
                    "Prefer code_index.get_index_status which includes richer metadata (file/symbol/reference counts and delta information)."
            };
            d.OutputSchema = new
            {
                type = "object",
                properties = new
                {
                    state = new { type = "string" }, latestSnapshotId = new { type = "string" },
                    lastCompletedAt = new { type = "string" }
                }
            };
        });
        ...
        Annotate("code_index.list_files", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    snapshotId = new { type = "string" }, items = new { type = "array" },
                    page = new { type = "integer" }, pageSize = new { type = "integer" },
                    totalItems = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        ...
        Annotate("code_index.get_file_outline", d =>
        {
            ...
            d.OutputSchema = new
            {
                type = "object", properties = new { filePath = new { type = "string" }, nodes = new { type = "array" } }
            };
        });
        ...
        Annotate("code_index.get_snapshot_info", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    snapshotId = new { type = "string" }, rootPath = new { type = "string" },
                    fileCount = new { type = "integer" }, symbolCount = new { type = "integer" },
        ...
        Annotate("code_index.list_roots", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                DoNotUseWhen =
                    "Prefer code_index.list_repositories which includes per-repo status (state, snapshot ID, file/symbol counts)."
            };
            d.OutputSchema = new
            {
                type = "object", properties = new { roots = new { type = "array", items = new { type = "string" } } }
            };
        });
        ...
        Annotate("code_index.find_symbols", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                TypicalNextTools = new[]
                    { "code_index.get_symbol", "code_index.get_symbol_occurrences", "code_index.get_dependencies" }
            };
            d.OutputSchema = new
            {
                type = "object",
        ...
        Annotate("code_index.get_symbol", d =>
        {
            ...
            d.UsageGuidance = new McpUsageGuidance
            {
                ...
                TypicalNextTools = new[]
                    { "code_index.get_symbol_occurrences", "code_index.get_dependencies", "code_index.get_references" }
            };
            d.OutputSchema = new
            {
                type = "object",
                properties = new
                {
                    snapshotId = new { type = "string" }, symbol = new { type = "object" },
                    primaryOccurrence = new { type = "object" },
                    error = new
                    {
                        type = "object",
                        properties = new
                        {
                            code = new { type = "string" }, message = new { type = "string" },
                            context = new { type = "object" }
                        }
                    }
                }
            };
        ...
        Annotate("code_index.get_symbol_occurrences", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    symbolId = new { type = "string" }, occurrences = new { type = "array" },
                    page = new { type = "integer" }, pageSize = new { type = "integer" },
                    totalItems = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        ...
        Annotate("code_index.get_symbol_children", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    symbolId = new { type = "string" }, children = new { type = "array" },
                    page = new { type = "integer" }, pageSize = new { type = "integer" },
                    totalItems = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        ...
        Annotate("code_index.get_type_members", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    typeSymbolId = new { type = "string" }, constructors = new { type = "array" },
                    methods = new { type = "array" }, properties = new { type = "array" },
                    fields = new { type = "array" }, events = new { type = "array" }
                }
            };
        });
        ...
        Annotate("code_index.resolve_location", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    filePath = new { type = "string" }, line = new { type = "integer" },
                    column = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("code_index.get_imports", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    filePath = new { type = "string" }, imports = new { type = "array" },
                    page = new { type = "integer" }, pageSize = new { type = "integer" },
                    totalItems = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        ...
        Annotate("code_index.get_file_references", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    filePath = new { type = "string" }, references = new { type = "array" },
                    page = new { type = "integer" }, pageSize = new { type = "integer" },
                    totalItems = new { type = "integer" }, hasMore = new { type = "boolean" }
                }
            };
        ...
        Annotate("code_index.get_dependencies", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    snapshotId = new { type = "string" }, symbolId = new { type = "string" },
                    dependencies = new { type = "array" }, page = new { type = "integer" },
                    pageSize = new { type = "integer" }, totalItems = new { type = "integer" },
                    hasMore = new { type = "boolean" }
                }
            };
        });
        ...
        Annotate("code_index.get_dependents", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                {
                    snapshotId = new { type = "string" }, symbolId = new { type = "string" },
                    dependents = new { type = "array" }, page = new { type = "integer" },
                    pageSize = new { type = "integer" }, totalItems = new { type = "integer" },
                    hasMore = new { type = "boolean" }
                }
            };
        ...
        Annotate("code_index.get_symbol_relationships", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                {
                    symbolId = new { type = "string" }, children = new { type = "array" },
                    references = new { type = "array" }, dependencies = new { type = "array" },
                    dependents = new { type = "array" }
                }
            };
        });
        ...
        Annotate("code_index.get_file_dependencies", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    filePath = new { type = "string" }, dependencyFiles = new { type = "array" },
                    totalEdges = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("code_index.update", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    snapshotId = new { type = "string" }, filesProcessed = new { type = "integer" },
                    symbolsAdded = new { type = "integer" }, symbolsRemoved = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("code_index.get_index_status", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    rootPath = new { type = "string" }, state = new { type = "string" },
                    latestSnapshotId = new { type = "string" }, fileCount = new { type = "integer" },
                    symbolCount = new { type = "integer" }
                }
            };
        });
        ...
        Annotate("code_index.clear_repository", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    rootPath = new { type = "string" }, removedSnapshots = new { type = "integer" },
        ...
        Annotate("code_index.ping", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    status = new { type = "string" }, service = new { type = "string" },
                    version = new { type = "string" },
                    languages = new { type = "array", items = new { type = "string" } },
                    tool_chains = new { type = "array", items = new { type = "string" } }
                }
            };
        });
        ...
        Annotate("repo_browser.print_tree", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    rootPath = new { type = "string" }, relativePath = new { type = "string" },
                    maxDepth = new { type = "integer" }, entryCount = new { type = "integer" },
                    truncated = new { type = "boolean" }, items = new { type = "array" }
                }
            };
            d.UsageGuidance = new McpUsageGuidance
            {
        ...
        Annotate("repo_browser.grep", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    rootPath = new { type = "string" }, query = new { type = "string" }, mode = new { type = "string" },
                    matchCount = new { type = "integer" }, truncated = new { type = "boolean" },
            ...
            {
                UseWhen =
                    "Use when you know what text, symbol, or pattern to search for but do not know which file contains it.",
                DoNotUseWhen =
                    "Do not use when you need file contents for a known file path or only need directory structure.",
                TypicalNextTools = new[]
                    { "repo_browser.read_file", "repo_browser.find_path", "repo_browser.print_tree" }
            };
        });
        ...
        Annotate("repo_browser.read_file", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    rootPath = new { type = "string" }, filePath = new { type = "string" },
                    exists = new { type = "boolean" }, encoding = new { type = "string" },
                    sizeBytes = new { type = "integer" }, truncated = new { type = "boolean" },
                    startLine = new { type = "integer" }, endLine = new { type = "integer" },
            ...
            {
                UseWhen =
                    "Use when you already know the exact file path and need to inspect its contents. Read partial ranges first for large files.",
                DoNotUseWhen =
                    "Do not use when the file path is still uncertain — use repo_browser.find_path or repo_browser.grep first.",
        ...
        Annotate("repo_browser.find_path", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                properties = new
                {
                    rootPath = new { type = "string" }, query = new { type = "string" },
                    matchMode = new { type = "string" }, targetKind = new { type = "string" },
                    resultCount = new { type = "integer" }, truncated = new { type = "boolean" },
                    items = new { type = "array" }
                }
            };
            d.UsageGuidance = new McpUsageGuidance
            {
                UseWhen =
                    "Use when you know the file or directory name (or part of its path) but not its exact location in the repository.",
        ...
        Annotate("repo_browser.search", d =>
        {
            ...
            d.OutputSchema = new
            {
                ...
                {
                    rootPath = new { type = "string" }, query = new { type = "string" },
                    pathPrefix = new { type = "string" }, resultCount = new { type = "integer" },
                    truncated = new { type = "boolean" }, items = new { type = "array" }
                }
            };
            d.UsageGuidance = new McpUsageGuidance
            {
                UseWhen =
                    "Use when you know part of a filename (e.g. \"Program\" to find Program.cs) but not its location. Searches all filenames by substring.",
                DoNotUseWhen =
                    "Do not use for content search (use repo_browser.grep) or to find directories (use repo_browser.find_path).",
                TypicalNextTools = new[] { "repo_browser.read_file", "repo_browser.print_tree", "repo_browser.grep" }
            };
        });
    }
        ...
        string[]? enums = null)
    {
        var prop = new McpPropertySchema
        {
            ...
            Description = description,
            Default = def,
            Enum = enums != null ? new List<string>(enums) : null!
        };
    ...
    private static List<string> StrList(Dictionary<string, object> args, string key)
    {
        ...
        if (val is JArray ja) return ja.ToObject<List<string>>()!;
        if (val is IEnumerable<object> en)
            return en.Select(x => x?.ToString()).Where(x => x != null).Select(x => x!).ToList();
    ...
    private static EditOptions EditOpts(Dictionary<string, object> args)
    {
        return new EditOptions
        {
            ...
            ChangeReason = Str(args, "change_reason"),
    ...
    {
        return ToResult(
            new { error = new McpError(code, message, context) });
    }
    ...
    private static string ClassifyError(string message)
    {
        ...
        {
            var m when m.Contains("not found", StringComparison.OrdinalIgnoreCase) => McpErrorCode.SymbolNotFound,
            var m when m.Contains("not indexed", StringComparison.OrdinalIgnoreCase) => McpErrorCode
                .RepositoryNotIndexed,
            var m when m.Contains("snapshot", StringComparison.OrdinalIgnoreCase) => McpErrorCode.SnapshotNotFound,
            var m when m.Contains("not supported", StringComparison.OrdinalIgnoreCase) => McpErrorCode
    ...
    private static bool GlobMatch(string str, string pat, int si, int pi)
    {
        while (pi < pat.Length)
        {
            if (pat[pi] == '*')
            {
                ...
                for (var i = si; i <= str.Length; i++)
                    if (GlobMatch(str, pat, i, pi))
                        return true;
                return false;
            }
    ...
    private static string AddLineNumbers(string content)
    {
        ...
        var sb = new StringBuilder(content.Length + lines.Length * 6);
