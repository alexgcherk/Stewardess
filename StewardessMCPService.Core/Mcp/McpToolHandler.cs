...
public sealed class McpToolHandler
{
    ...
    {
        return DispatchAsync(request, null, null, ct);
    }
    ...
    private McpResponse HandleInitialize(McpRequest request, string? existingSessionId)
    {
        ...
        var clientProtocol = ProtocolVersion;
        try
        {
    ...
    {
        return McpResponse.Ok(request.Id!, new McpPingResult
        {
            Status = "ok",
    ...
    private McpResponse HandleToolsList(McpRequest request)
    {
        ...
        try
        {
            var p = DeserializeParams<Dictionary<string, object>>(request.Params!);
        ...
        var nextCursor = nextOffset < tools.Count ? nextOffset.ToString() : null;
