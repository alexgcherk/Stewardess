...
public interface IIndexQueryService
{
    ...
    /// </summary>
    Task<GetImportsResponse> GetImportsAsync(GetImportsRequest request, int page = 1, int pageSize = 50,
        CancellationToken ct = default);

    /// <summary>
    ///     Returns outgoing and/or incoming reference edges for a single symbol.
    /// </summary>
    Task<GetReferencesResponse> GetReferencesAsync(GetReferencesRequest request, int page = 1, int pageSize = 50,
        CancellationToken ct = default);

    /// <summary>
    ///     Returns all reference edges originating in a single file.
    /// </summary>
    Task<GetFileReferencesResponse> GetFileReferencesAsync(GetFileReferencesRequest request, int page = 1,
    ...
    /// </summary>
    Task<GetDependenciesResponse> GetDependenciesAsync(GetDependenciesRequest request, int page = 1, int pageSize = 50,
        CancellationToken ct = default);

    /// <summary>
    ///     Returns inbound hard dependent projections for a symbol (symbols that depend on this one).
    /// </summary>
    Task<GetDependentsResponse> GetDependentsAsync(GetDependentsRequest request, int page = 1, int pageSize = 50,
        CancellationToken ct = default);
    ...
    /// </summary>
    Task<GetSymbolRelationshipsResponse> GetSymbolRelationshipsAsync(GetSymbolRelationshipsRequest request,
        CancellationToken ct = default);
    ...
    /// </summary>
    Task<GetFileDependenciesResponse> GetFileDependenciesAsync(GetFileDependenciesRequest request,
        CancellationToken ct = default);
}